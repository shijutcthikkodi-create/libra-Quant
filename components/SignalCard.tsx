...
import { ArrowUpRight, ArrowDownRight, Target, Cpu, Edit2, Check, X, TrendingUp, TrendingDown, Clock, ShieldAlert, Zap, AlertTriangle } from 'lucide-react';
...
const SignalCard: React.FC<SignalCardProps> = ({ signal, user, highlights }) => {
  ...
  const isTSLHit = isExited && (signal.comment?.toUpperCase().includes('TSL') || signal.status === TradeStatus.EXITED && (signal.pnlPoints || 0) > 0 && signal.comment?.toUpperCase().includes('TRAILING'));
  ...
  const getTargetStyle = (index: number) => {
    const isHit = (signal.targetsHit || 0) > index;
  ...
  const handleSaveTrail = () => {
    ...
    if (!isNaN(val)) setDisplayTrail(val); else setDisplayTrail(null);
      ...
      <div className="flex justify-between items-start p-5 pb-3">
        ...
        <div className="flex flex-col items-end space-y-1.5">
            <div className={`px-3 py-1 rounded text-[10px] font-bold border ${getStatusColor(signal.status)} flex items-center ${highlights?.has('status') ? 'animate-blink' : ''}`}>
                <span className={`w-1.5 h-1.5 rounded-full mr-2 ${isActive ? 'bg-current' : 'bg-current opacity-50'} ${isSLHit ? 'animate-ping' : ''}`}></span>
                {signal.status}
            </div>
            ...
            <div className="mt-auto pt-2 border-t border-slate-800/80">
                {isEditingTrail ? (
                    <div className="flex items-center space-x-1">
                        <input type="number" value={trailValue} onChange={(e) => setTrailValue(e.target.value)} className="w-full bg-slate-950 border border-blue-500/50 rounded text-[10px] px-2 py-1 text-white focus:outline-none font-mono" autoFocus />
      ...
      <div className="p-5">
        ...
        <div className="grid grid-cols-3 gap-2">
            {signal.targets && signal.targets.length > 0 ? signal.targets.map((t, idx) => {
                const isHit = (signal.targetsHit || 0) > idx;
                return (
        ...
        {signal.comment && (
            <div className={`mt-4 p-3 rounded-lg border transition-colors ${isSLHit || isTSLHit ? 'bg-rose-950/20 border-rose-500/30' : 'bg-slate-950/50 border-slate-800/50'} ${highlights?.has('comment') ? 'animate-blink' : ''}`}>
                <p className={`text-xs leading-relaxed ${isSLHit || isTSLHit ? 'text-rose-400 font-bold' : 'text-slate-400'}`}>
                  " {signal.comment} "
                </p>
            </div>
        )}
