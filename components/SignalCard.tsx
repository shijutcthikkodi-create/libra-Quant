...
const SignalCard: React.FC<SignalCardProps> = ({ signal, user, highlights, isMajorAlerting, onSignalUpdate, onSignalDelete, isRecentlyClosed }) => {
  ...
  const getTargetStyle = (index: number) => {
    ...
    // INSTITUTIONAL COLORING - SHADES OF GREEN
    switch (index) {
      case 0: // T1: Light Emerald
        return 'bg-[#10b981]/20 border-[#10b981] text-[#10b981] shadow-[0_0_12px_rgba(16,185,129,0.15)]';
      case 1: // T2: Medium Emerald
        return 'bg-[#059669]/40 border-[#059669] text-white shadow-[0_0_15px_rgba(5,150,105,0.25)]';
      case 2: // T3: Deep Emerald
        return 'bg-[#047857]/60 border-[#047857] text-white font-black shadow-[0_0_20px_rgba(4,120,87,0.35)]';
      default:
  ...
  const handleSaveTrail = async () => {
    ...
    if (onSignalUpdate) {
        ...
        const success = await onSignalUpdate({ ...signal, trailingSL: val, stopLoss: val !== null ? val : signal.stopLoss });
        if (success) { setDisplayTrail(val); setIsEditingTrail(false); }
  ...
  if (signal.targets && signal.targets.length > 0) {
    ...
    if (risk > 0) {
      riskReward = isBuy ? (signal.targets[0] - signal.entryPrice) / risk : (signal.entryPrice - signal.targets[0]) / risk;
    }
  }
    ...
    `}>
      ...
      {isBTST && (
        <div className="absolute top-0 right-0 z-20 overflow-hidden w-24 h-24 pointer-events-none">
          <div className="bg-amber-500 text-slate-950 text-[10px] font-black py-1 w-[140%] text-center transform rotate-45 translate-x-[20%] translate-y-[40%] shadow-lg border-b border-amber-600">OVERNIGHT</div>
        </div>
      )}
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <div className="flex items-center space-x-2 mb-0.5">
                <span className={`px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-widest ${isBuy ? 'bg-emerald-500 text-slate-950' : 'bg-rose-500 text-white'}`}>{signal.action}</span>
            ...
            <div className="flex items-center space-x-2 text-xs">
                ...
                <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${signal.type === OptionType.CE ? 'bg-green-900 text-green-300' : 'bg-red-900 text-red-300'}`}>{signal.type}</span>
            </div>
          </div>
        </div>
        <div className="flex flex-col items-end space-y-1.5 pr-2">
            <div className="flex items-center space-x-2">
              {user.isAdmin && onSignalDelete && (
                <button onClick={() => onSignalDelete(signal)} className="p-1.5 bg-slate-800 text-slate-500 hover:text-rose-500 rounded-lg border border-slate-700 transition-colors mr-1" title="Delete Signal"><Trash2 size={12} /></button>
              )}
      ...
      <div className="grid grid-cols-3 gap-px bg-slate-800 border-y border-slate-800">
        ...
        <div className={`p-4 flex flex-col transition-colors duration-500 ${isSLHit ? 'bg-rose-950/20' : 'bg-slate-900'} ${highlights?.has('stopLoss') || highlights?.has('trailingSL') ? 'animate-box-blink' : ''}`}>
            ...
            <p className={`text-xl font-mono font-bold mb-3 ${isSLHit ? 'text-rose-500 animate-pulse' : 'text-rose-400'}`}>₹{signal.stopLoss.toFixed(2)}</p>
            <div className={`mt-auto pt-2 border-t border-slate-800/80`}>
                {isEditingTrail ? (
                    ...
                    <div className={`flex items-center justify-between rounded -mx-1 px-1 py-1 transition-colors group/trail ${canEdit ? 'cursor-pointer hover:bg-slate-800/50' : 'opacity-70'}`} onClick={() => canEdit && setIsEditingTrail(true)}>
                         ...
                         <div className="flex items-center space-x-2">
                            <span className={`text-xs font-mono font-bold ${isTSLHit ? 'text-rose-500 animate-pulse' : 'text-yellow-500'}`}>{displayTrail != null ? `₹${displayTrail.toFixed(2)}` : '--'}</span>
      ...
      <div className="p-5">
            ...
            {signal.targets?.map((t, idx) => {
                ...
                return (
                  <div key={idx} className={`rounded px-2 py-2 text-center border transition-all duration-700 ${getTargetStyle(idx)}`}>
                      ...
                      <p className={`text-sm font-mono font-black ${isHit && isActive ? 'animate-pulse' : ''}`}>{t.toFixed(2)}</p>
        ...
        {signal.comment && (
            <div className={`mt-4 p-3 rounded-lg border transition-colors ${highlights?.has('comment') ? 'animate-box-blink' : ''} ${isSLHit || isTSLHit ? 'bg-rose-950/20 border-rose-500/30' : isAllTarget ? 'bg-emerald-950/20 border-emerald-500/30' : (isBTST ? 'bg-amber-950/20 border-amber-500/30' : 'bg-slate-950/50 border-slate-800/50')}`}>
                <p className={`text-xs leading-relaxed ${isSLHit || isTSLHit ? 'text-rose-400 font-bold' : isAllTarget ? 'text-emerald-400 font-bold italic' : (isBTST ? 'text-amber-400 font-bold' : 'text-slate-400')}`}>" {signal.comment} "</p>
            </div>
        )}
        <div className="mt-4 border-t border-slate-800 pt-3 flex justify-between items-center">
            <div className="flex items-center text-[10px] text-slate-600 font-mono"><Clock size={10} className="mr-1" />{new Date(signal.timestamp).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</div>
            <button onClick={handleAIAnalysis} disabled={loadingAnalysis} className="flex items-center py-1 text-[10px] font-bold text-blue-500 hover:text-blue-400 uppercase tracking-widest transition-colors"><Cpu size={12} className="mr-1.5" />{loadingAnalysis ? 'Syncing AI...' : analysis ? 'Close Intel' : 'AI Analysis'}</button>
        </div>
        {analysis && (
            <div className="mt-2 p-3 bg-slate-950 border border-blue-900/30 rounded text-[10px] text-slate-300 font-mono animate-in slide-in-from-top-2">
                <div className="text-blue-400 mb-1 font-bold uppercase tracking-widest text-[9px] border-b border-blue-900/30 pb-1 flex items-center"><Check size={10} className="mr-1" /> Quantitative Analysis Output</div>
                {analysis}
            </div>
        )}
      </div>
