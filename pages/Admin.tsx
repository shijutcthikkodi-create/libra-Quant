...
const Admin: React.FC<AdminProps> = ({ signals, users, logs = [] }) => {
  ...
  const handleAddSignal = async () => {
    ...
    // STRICT: Wait for sheet save. No local optimistic update.
    ...
    <div className="max-w-6xl mx-auto pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8">
        <div>
            ...
            <p className="text-slate-500 text-[10px] font-bold uppercase tracking-widest mt-1">Control Layer • Source: Google Sheets</p>
        </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-2xl">
                <div className="p-5 border-b border-slate-800 flex justify-between items-center bg-slate-800/20">
                    <div className="flex items-center">
                        ...
                        <h3 className="text-xs font-black text-white uppercase tracking-widest">Order Entry</h3>
                    </div>
                    {!isAddingSignal && (
                ...
                {isAddingSignal && (
                    <div className="p-6 bg-slate-950/40 space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
                            ...
                            <div>
                                <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5">Initial SL</label>
                            ...
                            <div className="md:col-span-2">
                                <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5">Targets (Comma separated)</label>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
                            ...
                            <div className="md:col-span-2">
                                <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5">Comment</label>
                                <input type="text" value={sigComment} onChange={e => setSigComment(e.target.value)} placeholder="Reasoning..." className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-2.5 text-xs text-white focus:border-blue-500 outline-none" />
                            </div>
                        ...
                        <div className="flex items-center space-x-3 pt-2">
                            ...
                            >
                                ...
                                {isSaving ? 'Broadcasting...' : 'Dispatch Signal'}
                            </button>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-2xl">
                <div className="p-5 border-b border-slate-800 bg-slate-800/10 flex justify-between items-center">
                    <div className="flex items-center">
                        ...
                        <h3 className="text-xs font-black text-white uppercase tracking-widest">Live Inventory ({activeSignals.length})</h3>
                    </div>
                ...
                <div className="p-5">
                    {activeSignals.length === 0 ? (
                        <div className="py-12 text-center border-2 border-dashed border-slate-800 rounded-2xl">
                            ...
                            <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest">No active positions in spreadsheet.</p>
                        </div>
                    ) : (
                            ...
                            {activeSignals.map((s) => (
                                <div key={s.id} className="bg-slate-950/50 border border-slate-800 rounded-2xl p-5 flex flex-col lg:row items-center justify-between gap-6 hover:border-slate-700 transition-all group">
                                    <div className="flex items-center space-x-4 w-full lg:w-auto">
                                        ...
                                        <div className="flex-1">
                                            <div className="flex items-center space-x-2 mb-0.5">
                                                <h4 className="font-mono font-black text-white uppercase tracking-tighter text-base">{s.instrument} {s.symbol}</h4>
                                            ...
                                            <div className="flex items-center space-x-3 text-[10px] font-bold text-slate-500">
                                                ...
                                                <span className="text-rose-500">SL: ₹{s.stopLoss}</span>
                                                {s.quantity ? <span className="text-blue-400">QTY: {s.quantity}</span> : null}
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 w-full lg:w-auto">
                                        <button 
                                            onClick={() => triggerQuickUpdate(s, { targetsHit: (s.targetsHit || 0) + 1, status: TradeStatus.PARTIAL, comment: `Target ${(s.targetsHit || 0) + 1} Done!` }, "TARGET")}
                                            className="px-3 py-2 bg-emerald-900/20 hover:bg-emerald-900/40 border border-emerald-500/20 text-emerald-400 rounded-xl text-[9px] font-black uppercase tracking-tighter transition-all"
                                        >
                                            Hit T{(s.targetsHit || 0) + 1}
                                        </button>

                                        <button 
                                            onClick={() => triggerQuickUpdate(s, { status: TradeStatus.ALL_TARGET, comment: "Excellent Trade! All targets achieved." }, "ALL_DONE")}
                                            className="px-3 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-[9px] font-black uppercase tracking-tighter transition-all shadow-lg"
                                        >
                                            All Targets
                                        </button>

                                        <button 
                                            onClick={() => triggerQuickUpdate(s, { status: TradeStatus.STOPPED, comment: "Strict Stop Loss hit." }, "SL_HIT")}
                                            className="px-3 py-2 bg-rose-900/20 hover:bg-rose-900/40 border border-rose-500/20 text-rose-400 rounded-xl text-[9px] font-black uppercase tracking-tighter transition-all"
                                        >
                                            SL Hit
                                        </button>

                                        <button 
                                            onClick={() => triggerQuickUpdate(s, { status: TradeStatus.EXITED, comment: "Positions closed at Market Price." }, "SQUARE_OFF")}
                                            className="px-3 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl text-[9px] font-black uppercase tracking-tighter transition-all"
                                        >
                                            Square Off
                                        </button>
                                    </div>
